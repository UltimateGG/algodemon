const fs = require('fs');
const loggers = require('./loggerMap.json');
...
const pipeLog = (prefix, msg, ...args) => {
  ...
  for (let key of Object.keys(loggers)) {
    if (!caller.includes(key)) continue;

    console[getFunc(prefix)](`[${prefix}] [${loggers[key]}]: ${msg}`, ...args);
    return writeToLog(`[${prefix}] [${loggers[key]}]: ${msg}`, ...args);
  }
