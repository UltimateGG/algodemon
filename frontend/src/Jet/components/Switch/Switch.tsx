...
const SwitchStyle = styled.input.attrs((props: SwitchProps) => props)`
  ...
  &::after {
    ...
    ${props => props.checked && `
      background-color: ${props.theme.colors[props.variant || 'primary'][0]};
  ...
  &:not([disabled]):hover {
    &::after {
      ...
      ${props => props.checked && `
        background-color: ${props.theme.colors[props.variant || 'primary'][2]};
      `}
    }
  }
  ...
  &:disabled {
    ...
    border: 2px solid ${props => props.theme.colors.background[3]};
    cursor: not-allowed;
    opacity: 0.5;
