...
const ButtonStyle = styled.button.attrs((props: ButtonProps) => props)`
    ...
    box-shadow: ${props => props.glowing && !props.glowOnHover && !props.disabled ? `-1px -1px ${props.glowAmount || '16px'} ${props.color === 'secondary' ? props.theme.colors.background[3] : props.theme.colors[props.color || 'primary'][0]}` : 'none'};
    transition: background-color 0.2s ease-in-out${props => props.glowOnHover ? ', box-shadow 0.4s ease-in-out' : ''};
    
    &:not([disabled]):active {
      background-color: ${props => {
        const col = props.color === 'secondary' ?  props.theme.colors.background[2] : props.theme.colors[props.color || 'primary'][3];
        if (props.variant === 'outlined') {
          const rgb = hexToRgb(col);
    ...
    &:not([disabled]):hover {
      background-color: ${props => {
        const col = props.color === 'secondary' ? props.theme.colors.background[props.variant === 'outlined' ? 1 : 2] : props.theme.colors[props.color || 'primary'][2];
        if (props.variant === 'outlined') {
          const rgb = hexToRgb(col);
      ...
      box-shadow: ${props => props.glowing || props.glowOnHover ? `-1px -1px ${props.glowAmount || '16px'} ${props.color === 'secondary' ? props.theme.colors.background[3] : props.theme.colors[props.color || 'primary'][0]}` : 'none'};
    }
`;
